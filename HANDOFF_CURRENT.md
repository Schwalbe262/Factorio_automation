...
- Planner fixes: refuel gear-mall iron source burner drill, repair unpowered iron-line inserters, obtain small-pole materials if needed, prefer direct gear->belt assembler inserter only when no active lane repair exists.
...
- Validation: targeted live-fix tests OK; `tests.test_strategy tests.test_planner tests.test_controller` 541 OK; full `unittest discover -s tests` 1028 OK.
- Token checkpoint: goal tracker `5,810,513`; part136 delta `1,312,779`; weekly quota unavailable; token sample recorded.
