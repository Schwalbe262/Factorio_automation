...
class ModlessLuaTests(unittest.TestCase):
    def test_observe_uses_silent_command_without_custom_mod_command(self):
        ...
        self.assertIn('"advanced-material-processing-2"', command)
        self.assertIn('"speed-module"', command)
        self.assertIn('"productivity-module"', command)
        self.assertIn('"efficiency-module"', command)
        self.assertIn('"effect-transmission"', command)
        self.assertIn("collect_recipe_unlocks", command)
        self.assertIn("recipe_unlocks = collect_recipe_unlocks()", command)
    ...
    def test_direct_feed_smelter_builder_places_two_to_four_cells_with_fuel_batch(self):
        ...
        self.assertIn("direct_feed_furnace_position", command)
        self.assertIn("direction_mismatch", command)
    ...
    def test_coal_bootstrap_cluster_builder_uses_dedicated_output_and_protected_teardown(self):
        ...
        self.assertIn("fuel_batch = clamp_int(params.fuel_count or params.fuel_batch, 24, 20, 30)", body)
        self.assertIn("builder_drill_center(resource_position)", command)
        self.assertIn("burner_drill_output_position_from(drill_position, drill_direction)", command)
